...
- **C3 분리**: 수집(C3a, 개발 PC) → HF 업로드 → 평가(C3b, GPU PC). held-out 추론은 GPU 필요이므로 개발 PC가 아닌 **어댑터·하네스 보유 PC(PC-1 또는 PC-3)**가 담당. `eval_hf_model.py --eval-file` 인자 **구현 완료**(임의 평가셋 CSV 지원). **C3b 재현율은 PC-1이 완료**(스미싱 24건: v1 0.667·v2 0.583). 남은 것=**정상 held-out 수집(C3a, 개발 PC)** → 확보 시 정밀도/FP 재측정(PC-1/3)
